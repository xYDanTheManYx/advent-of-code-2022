...
for crate_movement in crate_movements:
    ...
    for crate in range(crate_movement.number_of_crates):
        crates_to_move.append(stacks[crate_movement.from_stack-1].popleft())
    stacks[crate_movement.to_stack-1].extendleft(reversed(crates_to_move))

print("puzzle 2:", "".join([stack[0] for stack in stacks]))
